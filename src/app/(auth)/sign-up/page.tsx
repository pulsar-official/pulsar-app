...
export default function SignUpPage() {
  ...
  const [verifyStep, setVerifyStep] = useState<'email' | 'phone' | null>(null)
  ...
  const validate = () => {
    ...
    if (!phone.trim()) e.phone = 'Phone required for 2FA'
    else if (phone.replace(/D/g, '').length < 7) e.phone = 'Invalid phone number'
  ...
  const submit = async () => {
    ...
    try {
      ...
      await signUp.create({ emailAddress: email, password: pass, firstName: parts[0], lastName: parts.slice(1).join(' ') || undefined, phoneNumber: phone })
  ...
  const verifyEmail = async () => {
    ...
    try {
      ...
      else { await signUp.preparePhoneNumberVerification({ strategy: 'phone_code' }); setVerifyStep('phone') }
    } catch (err: unknown) {
  ...
  if (verifyStep === 'phone') {
    return (
      <div className="ps-auth" style={{ display:'flex',alignItems:'center',justifyContent:'center',minHeight:'100vh' }}>
        <div style={{ position:'relative',zIndex:1,width:400,padding:'36px 32px',background:'rgba(12,12,20,0.85)',backdropFilter:'blur(24px)',border:'1px solid var(--bd2)',borderRadius:18,boxShadow:'0 24px 80px rgba(0,0,0,0.5)' }}>
          <div style={{ display:'flex',alignItems:'center',gap:10,marginBottom:28 }}>
            <div style={{ width:32,height:32,borderRadius:9,background:'linear-gradient(135deg,#a78bfa,#7c3aed)',display:'grid',placeItems:'center',fontSize:14,fontWeight:700,color:'#fff' }}>P</div>
            <span style={{ fontWeight:700,fontSize:'1.15rem',letterSpacing:'-0.02em' }}>Pulsar</span>
          </div>
          <h1 style={{ fontSize:'1.5rem',fontWeight:700,letterSpacing:'-0.03em',marginBottom:8 }}>Verify your phone</h1>
          <p style={{ fontSize:'0.9rem',color:'var(--t2)',marginBottom:28,lineHeight:1.5 }}>We sent a code to <strong style={{ color:'var(--t1)' }}>{phone}</strong></p>
          <div style={{ marginBottom:16 }}>
            <label style={labelStyle}>SMS code</label>
            <input value={phoneCode} onChange={e=>{setPhoneCode(e.target.value);setErrors({})}} placeholder="Enter 6-digit code" style={inputStyle('phoneCode')}
              onFocus={e=>{e.currentTarget.style.borderColor='var(--ac)';e.currentTarget.style.boxShadow='0 0 0 3px rgba(167,139,250,0.08)'}}
              onBlur={e=>{e.currentTarget.style.borderColor='var(--bd2)';e.currentTarget.style.boxShadow='none'}}
              onKeyDown={e=>{if(e.key==='Enter')verifyPhone()}}
            />
            {errors.phoneCode && <span style={{ fontSize:'0.72rem',color:'var(--err)',fontFamily:'var(--mn)',marginTop:4,display:'block' }}>{errors.phoneCode}</span>}
          </div>
          <button onClick={verifyPhone} disabled={loading} style={{ width:'100%',padding:14,borderRadius:10,border:'none',background:loading?'var(--s4)':'linear-gradient(135deg,#a78bfa,#7c3aed)',color:'#fff',fontSize:'0.98rem',fontWeight:600,cursor:loading?'wait':'pointer',fontFamily:'var(--ft)' }}>{loading?'Verifying...':'Verify Phone'}</button>
          <p style={{ textAlign:'center',marginTop:16,fontSize:'0.85rem',color:'var(--t3)' }}>Wrong number? <a href="/sign-up" style={{ color:'var(--ac)',textDecoration:'none',fontWeight:600 }}>Start over</a></p>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="ps-auth" style={{ display:'flex',alignItems:'center',justifyContent:'center',minHeight:'100vh',position:'relative' }}>
      ...
      <div style={{ position:'relative',zIndex:1,width:420,padding:'36px 32px',background:'rgba(12,12,20,0.85)',backdropFilter:'blur(24px)',border:'1px solid var(--bd2)',borderRadius:18,boxShadow:'0 24px 80px rgba(0,0,0,0.5)',animation:`psFadeIn 0.6s ${E}` }}>
        ...
        <h1 style={{ fontSize:'1.6rem',fontWeight:700,letterSpacing:'-0.03em',marginBottom:6 }}>Create your account</h1>
        <p style={{ fontSize:'0.92rem',color:'var(--t2)',marginBottom:28,lineHeight:1.5 }}>Start free. No credit card needed.</p>
        {/* OAuth */}
        ...
        <div style={{ animation:shake?'psShake 0.4s ease':'none' }}>
          ...
          <div style={{ marginBottom:16 }}>
            <label style={labelStyle}>Phone <span style={{ fontSize:'0.65rem',fontFamily:'var(--mn)',color:'var(--t3)',fontWeight:400,marginLeft:4 }}>2FA</span></label>
            <input type="tel" value={phone} onChange={e=>{setPhone(e.target.value);setErrors(er=>{const n={...er};delete n.phone;return n})}} placeholder="+1 (555) 000-0000" style={inputStyle('phone')}
              onFocus={e=>{if(!errors.phone)e.currentTarget.style.borderColor='var(--ac)';e.currentTarget.style.boxShadow='0 0 0 3px rgba(167,139,250,0.08)'}}
              onBlur={e=>{if(!errors.phone)e.currentTarget.style.borderColor='var(--bd2)';e.currentTarget.style.boxShadow='none'}}
            />
            {errors.phone&&<span style={{ fontSize:'0.72rem',color:'var(--err)',fontFamily:'var(--mn)',marginTop:4,display:'block' }}>{errors.phone}</span>}
          </div>
        ...
        <p style={{ textAlign:'center',marginTop:24,fontSize:'0.88rem',color:'var(--t3)' }}>
          Already have an account? <a href="/sign-in" style={{ color:'var(--ac)',textDecoration:'none',fontWeight:600 }}>Sign in</a>
        </p>
      </div>
    </div>
  )
