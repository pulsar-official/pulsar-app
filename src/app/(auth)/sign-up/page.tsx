...
export default function SignUpPage() {
  ...
  const verifyEmail = async () => {
    setLoading(true)
    try {
      const { error } = await supabase.auth.verifyOtp({ type: 'signup', email, token: emailCode.trim() })
      if (error) {
  ...
  const resendCode = async () => {
    ...
    try {
      ...
      setErrors({ emailCode: 'Failed to resend' })
